...
port (
    ...
    output_r_d0           :in   STD_LOGIC_VECTOR(7 downto 0);
...
architecture behave of radix_control_s_axi is
    ...
    constant ADDR_INPUT_R_BASE  : INTEGER := 16#10#;
    constant ADDR_INPUT_R_HIGH  : INTEGER := 16#17#;
    constant ADDR_OUTPUT_R_BASE : INTEGER := 16#18#;
    constant ADDR_OUTPUT_R_HIGH : INTEGER := 16#1f#;
    ...
    -- memory signals
    signal int_input_r_address0 : UNSIGNED(0 downto 0);
    signal int_input_r_ce0     : STD_LOGIC;
    signal int_input_r_q0      : UNSIGNED(31 downto 0);
    signal int_input_r_address1 : UNSIGNED(0 downto 0);
    ...
    signal int_input_r_shift0  : UNSIGNED(1 downto 0);
    signal int_output_r_address0 : UNSIGNED(0 downto 0);
    signal int_output_r_ce0    : STD_LOGIC;
    signal int_output_r_be0    : UNSIGNED(3 downto 0);
    signal int_output_r_d0     : UNSIGNED(31 downto 0);
    signal int_output_r_address1 : UNSIGNED(0 downto 0);
    ...
    signal int_output_r_shift0 : UNSIGNED(1 downto 0);

    component radix_control_s_axi_ram is
        generic (
    ...
    end component radix_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_input_r
int_input_r : radix_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 4,
...
port map (
     ...
     q1        => int_input_r_q1);
-- int_output_r
int_output_r : radix_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "S2P",
     BYTES     => 4,
...
-- ----------------------- Memory logic ------------------
    -- input_r
    int_input_r_address0 <= SHIFT_RIGHT(UNSIGNED(input_r_address0), 2)(0 downto 0);
    int_input_r_ce0      <= input_r_ce0;
    input_r_q0           <= STD_LOGIC_VECTOR(SHIFT_RIGHT(int_input_r_q0, TO_INTEGER(int_input_r_shift0) * 8)(7 downto 0));
    int_input_r_address1 <= raddr(2 downto 2) when ar_hs = '1' else waddr(2 downto 2);
    ...
    -- output_r
    int_output_r_address0 <= SHIFT_RIGHT(UNSIGNED(output_r_address0), 2)(0 downto 0);
    int_output_r_ce0     <= output_r_ce0;
    int_output_r_be0     <= SHIFT_LEFT(TO_UNSIGNED(1, 4), TO_INTEGER(UNSIGNED(output_r_address0(1 downto 0)))) when output_r_we0 = '1' else (others=>'0');
    int_output_r_d0      <= UNSIGNED(output_r_d0) & UNSIGNED(output_r_d0) & UNSIGNED(output_r_d0) & UNSIGNED(output_r_d0);
    int_output_r_address1 <= raddr(2 downto 2) when ar_hs = '1' else waddr(2 downto 2);
        ...
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (input_r_ce0 = '1') then
                    int_input_r_shift0 <= UNSIGNED(input_r_address0(1 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (output_r_ce0 = '1') then
                    int_output_r_shift0 <= UNSIGNED(output_r_address0(1 downto 0));
                end if;
            end if;
        end if;
    end process;
...
entity radix_control_s_axi_ram is
    generic (
        MEM_STYLE : STRING :="auto";
        MEM_TYPE  : STRING :="S2P";
        BYTES   : INTEGER :=4;
...
end entity radix_control_s_axi_ram;

architecture behave of radix_control_s_axi_ram is
