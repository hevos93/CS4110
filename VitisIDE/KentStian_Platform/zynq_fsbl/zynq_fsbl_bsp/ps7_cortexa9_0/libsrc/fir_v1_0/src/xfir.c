...
void XFir_Set_input_r(XFir *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XFir_WriteReg(InstancePtr->Control_BaseAddress, XFIR_CONTROL_ADDR_INPUT_R_DATA, Data);
}

u32 XFir_Get_input_r(XFir *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XFir_ReadReg(InstancePtr->Control_BaseAddress, XFIR_CONTROL_ADDR_INPUT_R_DATA);
    return Data;
}

u32 XFir_Get_output_r(XFir *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XFir_ReadReg(InstancePtr->Control_BaseAddress, XFIR_CONTROL_ADDR_OUTPUT_R_DATA);
    return Data;
}

u32 XFir_Get_output_r_vld(XFir *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XFir_ReadReg(InstancePtr->Control_BaseAddress, XFIR_CONTROL_ADDR_OUTPUT_R_CTRL);
    return Data & 0x1;
}
